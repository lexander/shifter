...
/*
 * Checks if this is the first time that Shifter has built
 * a set of modules into the specified build directory by
 * checking for the existence of the build directory.
 */
var isFirstBuild = function(options, args) {

    // local re-evaluation of options to get
    // correct build directory
    options['walk'] = true;
    options['build-dir'] = args['build-dir'];
    options = args.defaults(options);

    if (fs.existsSync(options['build-dir']) && fs.statSync(options['build-dir']).isDirectory()){
        log.info('found existing build dir:' + options['build-dir']);
        return false;
    };
...
exports.init = function (opts, initCallback) {
    ...
    if (options.watch) {
        // check for first-time build and build the whole project
        // if appropriate
        if (isFirstBuild(options, args)) {
            log.info('first build with --watch, running full build to init project');
