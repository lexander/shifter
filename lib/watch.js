...
var handler = function (file, curr, prev) {
    ...
    if (usableExts[ext]) {
        if (changed(curr, prev)) {
            log.info("looking for build.json in: " + dir);
 ...
 */
var isFirstBuild = function() {
  var fs = require('fs'),
      args = require('./args'),
      options = args.defaults();
  // its called 'defaults' but it actually checks the user's argv

  log.info('looking for build dir:' + options['build-dir']);
...
exports.start = function (options) {
    ...
    var start = function() {
        ...
        if(isFirstBuild()) {
          ...
          log.info('first build, walking the directories');
          walk.run(buildArgs, function(){
